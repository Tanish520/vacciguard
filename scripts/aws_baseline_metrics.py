...
def _parse_replay_metrics(replay_logs, metrics):
...
def _parse_stream_summary_metrics(stream_logs):
...
def _parse_stream_metrics_payload(stream_metrics_payload):
...
def main() -> None:
    ...
    parser.add_argument("--stream-metrics", default="")
    ...
    stream_metrics_payload = (
        Path(args.stream_metrics).read_text(encoding="utf-8")

...
class ReplayMetricsRegistry:
    def __init__(self):
        ...
        self._metrics = {
            ...
            "vacciguard_replay_kafka_ack_latency_sum_seconds": 0.0,
            "vacciguard_replay_kafka_ack_latency_count": 0,
            "vacciguard_replay_send_errors_total": 0,
        }
    ...
    def begin_run(self, configured_events_per_second, run_started_timestamp_seconds=None):
        with self._lock:
            ...
            self._metrics["vacciguard_replay_kafka_ack_latency_sum_seconds"] = 0.0
            self._metrics["vacciguard_replay_kafka_ack_latency_count"] = 0
            self._metrics["vacciguard_replay_send_errors_total"] = 0

    def record_sent_event(self, duration_seconds=None, kafka_ack_latency_seconds=None):
        with self._lock:
            ...
            if kafka_ack_latency_seconds is not None:
                self._metrics["vacciguard_replay_kafka_ack_latency_sum_seconds"] += (
                    kafka_ack_latency_seconds
                )
                self._metrics["vacciguard_replay_kafka_ack_latency_count"] += 1

    def record_send_error(self):
        with self._lock:
            self._metrics["vacciguard_replay_send_errors_total"] += 1
    ...
    for attempt in range(1, retries + 1):
        try:
            producer = KafkaProducer(
                ...
                compression_type="gzip",  # compress to reduce network I/O
                max_in_flight_requests_per_connection=5,  # allow in-flight sends
...
def encode_event_payload(event, sent_at=None):
    ...
    if sent_at is None:
        replay_sent_at = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
    else:
        replay_sent_at = sent_at
    stamped_event["replay_sent_at"] = replay_sent_at
    return json.dumps(stamped_event, separators=(",", ":")).encode("utf-8")

# ── Replay ────────────────────────────────────────────────────────────────────

# Batch size for flushing Kafka futures. This is the key throughput optimization:
# instead of waiting for every single send to complete, we keep N futures in flight
# and only wait when the batch is full. This allows Kafka's internal batching to work.
REPLAY_FUTURE_BATCH_SIZE = int(os.environ.get("REPLAY_FUTURE_BATCH_SIZE", "200"))
...
def replay(producer, events, eps, metrics_registry=None):
    ...
    Throughput optimization: instead of waiting for every send_future.get()
    (which serializes each message), we batch futures and flush periodically.
    This allows Kafka's internal batching and parallelism to work properly.
    """
    interval  = 1.0 / eps
    total     = len(events)
    sent      = 0
    ...
    completion_status = 1
    in_flight_futures = []  # list of (future, send_timestamp) tuples
    ...
    try:
        for event in events:
            ...
            in_flight_futures.append((send_future, send_ts))

            # Only wait for futures when the batch is full.
            # This allows Kafka to batch multiple messages into a single request.
            if len(in_flight_futures) >= REPLAY_FUTURE_BATCH_SIZE:
                _flush_futures(in_flight_futures, metrics_registry)
                in_flight_futures = []
        ...
        # Flush any remaining futures after the loop
...
def _flush_futures(futures_list, metrics_registry=None):
    """Wait for all in-flight futures and record Kafka ack latency metrics.

    This is the key difference from the old approach: instead of waiting for
    each individual send, we batch the waits. Kafka's internal batching means
    these futures often resolve together, dramatically improving throughput.
    """
    for future, send_ts in futures_list:
        try:
            ack_start = time.monotonic()
            future.get(timeout=30)
            kafka_ack_latency = time.monotonic() - ack_start
            if metrics_registry is not None:
                metrics_registry.record_sent_event(
                    kafka_ack_latency_seconds=kafka_ack_latency,
                )
        except Exception:
            if metrics_registry is not None:
                metrics_registry.record_send_error()

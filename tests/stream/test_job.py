...
class BatchSummaryBehaviorTests(unittest.TestCase):
    ...
    def test_write_batch_summary_updates_stream_metrics_registry(self):
        ...
        # The new implementation calls update_batch_metrics with additional parameters
        call_kwargs = mock_registry.update_batch_metrics.call_args.kwargs
        self.assertEqual(call_kwargs["batch_id"], 14)
        self.assertEqual(call_kwargs["processed_count"], 1)
        self.assertEqual(call_kwargs["invalid_count"], 1)
        self.assertEqual(call_kwargs["deduplicated_count"], 1)
        self.assertEqual(call_kwargs["breach_count"], 0)
        self.assertIsNone(call_kwargs["avg_latency_seconds"])
        self.assertIsNone(call_kwargs["p95_latency_seconds"])
        # New fields should be present
        self.assertIn("batch_processing_duration_seconds", call_kwargs)
        self.assertIn("ingest_delay_avg_seconds", call_kwargs)
        self.assertIn("ingest_delay_p95_seconds", call_kwargs)
...
class ProcessedBatchWriteTests(unittest.TestCase):
    ...
    def test_write_processed_batch_appends_parquet_and_updates_redis(self):
        ...
        ), patch.object(stream_job, "write_latest_state_to_redis", return_value=(0.0, 1)):
